import { sliderSetTimerange } from "./iottimeseriesData";
...
export const overviewToggle = (deviceType, device) => ({
  type: TOGGLE,
  deviceType: deviceType,
  device: device
});
export const randomizeChartData = data => ({
  type: GENERATE_RANDOM_DATA,
  data: data
});
export const zoom = InOut => ({ type: MANAGE_ZOOM, InOut: InOut });
export const manageDrawerOpen = open => ({
  type: MANAGE_DRAWER_OPEN,
  open: open
});
export const manageDialogOpen = (
  open,
  deviceName,
  deviceTitle,
  deviceSection,
...
export const manageDialogTab = index => ({
  type: MANAGE_DIALOG_TAB,
  index: index
});
export const setCurrentDeviceStatus = status => ({
  type: SET_CURRENT_DEVICE_STATUS,
  status: status
});
export const setCurrentDeviceType = deviceType => ({
...
export const displayTimeRange = (
  ...
  return async function(dispatch, getState) {
    try {
      ...
      dispatch(manageDialogTab(tabName));
      dispatch(sliderSetTimerange(timerangeISOString));
    } catch (err) {
      await dispatch(
