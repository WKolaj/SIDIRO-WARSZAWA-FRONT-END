...
export const getData = (
  ...
  return dispatch => {
    ...
    switch (tabIndex) {
      case "voltageLLTab":
        variablesToGET = ["Voltage_L1_L2", "Voltage_L2_L3", "Voltage_L3_L1"];
        break;
      case "voltageLNTab":
        variablesToGET = ["Voltage_L1_N", "Voltage_L2_N", "Voltage_L3_N"];
        break;
      case "currentTab":
      ...
      case "powerTab":
        if (
          ...
          variablesToGET = [
            "Total_active_power_import",
            "Total_reactive_power_import",
            "Total_apparent_power",
            "Total_active_power_export",
            "Total_reactive_power_export"
          ];
        } else {
          variablesToGET = [
            "Active_power_import_15_min",
            "Reactive_power_import_15_min",
            "Active_power_export_15_min",
            "Reactive_power_export_15_min"
          ];
        }
        break;
      case "THDUtab":
            ...
            if (key.indexOf("_qc") === -1 && key.indexOf("_time") === -1) {
              ...
              res.data.map(singlePoint => {
                return xyData.unshift({
                  ...
                  y:
                    tabIndex === "powerTab"
                      ? parseFloat((singlePoint[key] / 1000).toFixed(3))
                      : parseFloat(singlePoint[key].toFixed(3))
                });
              });
