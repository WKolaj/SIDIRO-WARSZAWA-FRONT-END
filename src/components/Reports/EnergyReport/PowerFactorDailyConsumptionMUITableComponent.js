...
class DailyConsumptionTableComponent extends Component {
  getColumns = () => {
    ...
    columns.push({
      ...
      field: `total.powerFactorImport`,
      render: rowData => (
        <span className={classes.valueField}>
          {rowData["total"]
            ? `${rowData["total"].powerFactorImport.toFixed(3)}`
            : ""}
        </span>
      )
    });
    ...
    columns.push({
      ...
      field: `total.powerFactorExport`,
      render: rowData => (
        <span className={classes.valueField}>
          {rowData["total"]
            ? `${rowData["total"].powerFactorExport.toFixed(3)}`
            : ""}
        </span>
      )
    });
    ...
    return (
      <MaterialTable
        ...
        options={{
          ...
          padding: "dense",
          exportButton: true,
          exportFileName: "powerFactorConsumptionDaily.csv"
        }}
        localization={{
          pagination: {
