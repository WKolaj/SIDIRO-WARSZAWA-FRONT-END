  ...
  getColumns = groupsWithoutTotal => {
    ...
    for (let group of groupsWithoutTotal) {
      columns.push({
        ...
        field: `${group}.reactiveEnergyExport`,
        render: rowData => (
          <span className={classes.valueField}>
            {rowData[group]
              ? `${(rowData[group].reactiveEnergyExport / 1000).toFixed(2)}`
              : ""}
          </span>
        )
      });
    ...
    columns.push({
      ...
      field: `total.reactiveEnergyExport`,
      render: rowData => (
        <span className={classes.valueField}>
          {rowData["total"]
            ? `${(rowData["total"].reactiveEnergyExport / 1000).toFixed(2)}`
            : ""}
        </span>
      )
    });
    ...
    return (
      <MaterialTable
        ...
        options={{
          ...
          padding: "dense",
          exportButton: true,
          exportFileName: "reactiveExportEnergyConsumptionDaily.csv"
        }}
        localization={{
          pagination: {
