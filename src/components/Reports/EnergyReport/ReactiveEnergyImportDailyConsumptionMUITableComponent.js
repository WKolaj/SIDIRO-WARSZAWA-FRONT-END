...
class DailyConsumptionTableComponent extends Component {
  getColumns = groupsWithoutTotal => {
    ...
    for (let group of groupsWithoutTotal) {
      columns.push({
        title: t(`reportsEnergyReportGroupName_${group}`) + " [kvarh]",
        field: `${group}.reactiveEnergyImport`,
        render: rowData => (
          <span className={classes.valueField}>
            {rowData[group]
              ? `${(rowData[group].reactiveEnergyImport / 1000).toFixed(2)}`
              : ""}
          </span>
        )
      });
    ...
    columns.push({
      title: t(`reportsEnergyReportGroupName_total`) + " [kvarh]",
      field: `total.reactiveEnergyImport`,
      render: rowData => (
        <span className={classes.valueField}>
          {rowData["total"]
            ? `${(rowData["total"].reactiveEnergyImport / 1000).toFixed(2)}`
            : ""}
        </span>
      )
    });
    ...
    return (
      <MaterialTable
        ...
        options={{
          ...
          padding: "dense",
          exportButton: true,
          exportFileName: "reactiveImportEnergyConsumptionDaily.csv"
        }}
        localization={{
          pagination: {
